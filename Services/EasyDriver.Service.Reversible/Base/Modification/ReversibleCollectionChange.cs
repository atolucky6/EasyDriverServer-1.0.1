...
{
    public class ReversibleCollectionChange<TItem> : ReversibleChange
    {
        ...
        public ReversibleCollectionChange(CollectionAction action, IList<TItem> source, TItem changedItem, int oldIndex)
        {
            this.action = action;
            this.source = source;
            changedItems = new List<TItem>() { changedItem };
            indexDictionary = new Dictionary<TItem, int>();
            foreach (var item in changedItems)
                indexDictionary.Add(item, oldIndex);
        }
