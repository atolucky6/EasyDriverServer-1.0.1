...
namespace EasyDriver.Service.ProjectManager
{
    public class CoreItemJsonConverter : JsonConverter
    {
        ...
        public IEasyDriverPlugin CurrentDriver { get; set; }
        public IGroupItem CurrentParent { get; set; }

        public CoreItemJsonConverter(IDriverManagerService driverManagerService, IRemoteConnectionManagerService remoteConnectionManagerService)
        {
        ...
        {
            return typeof(ICoreItem).IsAssignableFrom(objectType);
        }
            ...
            if (serializer.Deserialize(reader) is JObject jObject)
            {
                ...
                switch (itemType)
                {
                    ...
                    case ItemType.RemoteStation:
                        var remotePlugin = RemoteConnectionManagerService.CreateRemoteConnection(jObject["StationType"].Value<string>());
                        if (remotePlugin != null)
                            result = remotePlugin.CreateRemoteConnection(CurrentParent);
                        break;
                    case ItemType.Channel:
                        CurrentDriver = DriverManagerService.CreateDriver(jObject["DriverPath"].Value<string>());
                        if (CurrentDriver != null)
                            result = CurrentDriver.CreateChannel(CurrentParent);
                        break;
                    case ItemType.Device:
                        if (CurrentDriver != null)
                            result = CurrentDriver.CreateDevice(CurrentParent);
                        break;
                    case ItemType.Group:
                        result = new GroupCore(CurrentParent, false);
                        break;
                    case ItemType.Tag:
                        if (!jObject["IsInternalTag"].Value<bool>())
                        {
                            if (CurrentDriver != null)
                                result = CurrentDriver.CreateTag(CurrentParent);
                        }
                        else
                        {
                            result = new TagCore(CurrentParent);
                        }
                        break;
                        ...
                        case ItemType.Tag:
                            {
                                ...
                                tag.EnabledWriteLimit = jObject["Retain"].Value<bool>();
                                tag.WriteMaxLimit = jObject["WriteMaxLimit"].Value<double>();
                                tag.WriteMinLimit = jObject["WriteMinLimit"].Value<double>();
                                tag.Unit = jObject["Unit"].Value<string>();
                                tag.DefaultValue = jObject["DefaultValue"].Value<string>();
                                break;
                            }
                        case ItemType.ConnectionSchema:
                            {
                                IProjectItem project = result as IProjectItem;
        ...
        public override void WriteJson(JsonWriter writer, object value, JsonSerializer serializer)
        {
            ...
            if (value != null && value is IGroupItem groupItem)
            {
                ...
                else if (value is ITagCore tag)
                {
                    ...
                    writer.WritePropertyName("EnabledWriteLimit");
                    writer.WriteValue(tag.EnabledWriteLimit);

                    writer.WritePropertyName("WriteMinLimit");
                    writer.WriteValue(tag.WriteMinLimit);

                    writer.WritePropertyName("WriteMaxLimit");
                    writer.WriteValue(tag.WriteMaxLimit);

                    writer.WritePropertyName("DefaultValue");
                    writer.WriteValue(tag.DefaultValue);

                    writer.WritePropertyName("Unit");
                    writer.WriteValue(tag.Unit);
                }
                else if (value is IProjectItem project)
                {
