...
{
    [Service(0, true)]
    public class TagWriterService : EasyServicePluginBase, ITagWriterService
    {
        #region Constructors

        public TagWriterService() : base()
        {
                ...
                if (!string.IsNullOrWhiteSpace(writeCommand.Prefix) && !string.IsNullOrWhiteSpace(writeCommand.TagName))
                {
                    ...
                    else
                    {
                        ...
                        if (pathSplit != null && pathSplit.Length > 0)
                        {
                            ...
                            {
                                if (RemoteConnectionManagerService.RemoteConnectionPool.ContainsKey(stationCore))
                                {
                                    string oldPath = writeCommand.Prefix;
                                    IEasyRemoteConnectionPlugin remoteConnection = RemoteConnectionManagerService.GetRemoteConnection(stationCore);
                                    writeCommand.Prefix = writeCommand.Prefix.Remove(0, pathSplit[0].Length + 1);
                                    respone = await remoteConnection.WriteAsync(writeCommand);
