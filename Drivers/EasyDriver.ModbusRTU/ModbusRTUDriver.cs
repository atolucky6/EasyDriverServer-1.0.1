...
namespace EasyDriver.ModbusRTU
{
    ...
    public class ModbusRTUDriver : EasyDriverPluginBase
    {
        ...
        private ModbusSerialRTU mbMaster;
                    ...
                    if (Channel.Enabled)
                    {
                        ...
                        if (mbMaster.Open())
                        {
                            ...
                            foreach (var channelChild in Channel.GetAllDevices().ToArray())
                            {
                                ...
                                {
                                    case ReadMode.Single:
                                        foreach (var childDevice in device.ChildTags.ToArray())
                                        {
                                    ...
                                    case ReadMode.Auto:
                                        {
                                            foreach (ReadBlockSetting block in device.AutoReadBlockSettings.ToArray())
                                            {
                                                block.ReadResult = false;
                                                if (block.IsValid && block.Enabled && block.AddressType != AddressType.Undefined)
                                                {
                        ...
                        // Ghi Quality của tất cả tag trong channel thành Bad
                        SET_TAG_BAD:
                        if (setTagBad)
                        {
