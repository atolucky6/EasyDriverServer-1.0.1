...
namespace EasyDriver.ModbusRTU
{
    ...
    public class ReadBlockSetting : INotifyPropertyChanged, IDataErrorInfo
    {
        ...
        public bool TryAddTag(Tag tag, out int index)
        {
            index = 0;
            if (tag != null && tag.AddressType == this.AddressType)
            {
                int maxReadLen = 125;
                if (tag.AddressType == AddressType.InputContact ||
                    tag.AddressType == AddressType.OutputCoil)
                    maxReadLen = 2000;

                int requieEndOffset = tag.AddressOffset + tag.RequireByteLength - 1;
                int newStartOffset = StartOffset;
                int newEndOffset = EndOffset;
                if (tag.AddressOffset < StartOffset)
                {
                    newStartOffset = tag.AddressOffset;                    
                }

                if (requieEndOffset > EndOffset)
                    newEndOffset = requieEndOffset;

                int readLen = newEndOffset - newStartOffset;
                if (readLen < maxReadLen)
                {
                    BeginEdit();
                    StartAddress = ((int)AddressType * 100000 + newStartOffset).ToString();
                    EndAddress = ((int)AddressType * 100000 + newEndOffset).ToString();
