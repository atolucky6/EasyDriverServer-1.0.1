...
namespace EasyDriver.ModbusRTU
{
    public class Tag : TagCore
    {
        ...
        public Device DeviceParent { get; set; }

        public ReadBlockSetting ReadBlockSetting { get; set; }
        public ReadBlockSetting AutoReadBlockSetting { get; set; }

        public int IndexOfDataInBlockSetting { get; set; }
        public int IndexOfDataInAutoBlockSetting { get; set; }
        ...
        public void FindAndRegisterReadBlock()
        {
            if (DeviceParent != null)
            {
                ...
                {
                    foreach (ReadBlockSetting setting in DeviceParent.GetReadBlockSettings(AddressType).ToArray())
                    {
                        if (setting.IsTagInRange(this, out int index))
                        {
                            UnregisterReadBlock();
                            RegisterReadBlock(DeviceParent, setting, index);
                            return;
                        }
                    }
                }
                ...
                if (!DeviceParent.UndefinedTags.Contains(this))
                    DeviceParent.UndefinedTags.Add(this);
            }
        }
        ...
        private void OnItemWasRemoveFromParent(object sender, EventArgs e)
        {
            ...
            {
                DeviceParent.UndefinedTags.Remove(this);
                DeviceParent.ChildTags.Remove(this);
            }
            DeviceParent = null;
        }
        ...
        {
            DeviceParent = this.FindParent<Device>(x => x is Device);
            if (DeviceParent != null)
                DeviceParent.ChildTags.Add(this);
