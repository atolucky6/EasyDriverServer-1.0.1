...
{
    public class OpenProjectViewModel : ISupportParentViewModel
    {
        ...
        protected IProjectManagerService ProjectManagerService { get => ServicePlugin.ServiceContainer.Default.GetService<IProjectManagerService>(); }
        protected IWorkspaceManagerService WorkspaceManagerService { get => ServicePlugin.ServiceContainer.Default.GetService<IWorkspaceManagerService>(); }
        protected IReversibleService ReverseService { get => ServicePlugin.ServiceContainer.Default.GetService<IReversibleService>(); }
        protected IApplicationPropertiesService ApplicationPropertiesService { get => ServicePlugin.ServiceContainer.Default.GetService<IApplicationPropertiesService>(); }
        #endregion
        ...
        public virtual void OnLoaded()
        {
            ...
            if (ApplicationPropertiesService.TryGetValue("RecentOpenProjects", out string jsonResult))
            {
                List<RecentOpenProjectModel> recentProjects = JsonConvert.DeserializeObject<List<RecentOpenProjectModel>>(jsonResult);
                if (recentProjects != null)
                    recentProjects.ForEach(x => RecentOpenProjects.Add(x));
            }
        }
        ...
        public virtual void OnUnloaded()
        {
            ...
            ApplicationPropertiesService.SetValue("RecentOpenProjects", JsonConvert.SerializeObject(RecentOpenProjects.ToList(), Formatting.Indented));
            ...
            try
            {
                if (projectInfo != null && !IsBusy)
                {
                    ...
                    else
                    {
                        ...
                        IsBusy = true;
                        IProjectItem openedProject = await ProjectManagerService.OpenProjectAsync(SelectedItem.Path);
        ...
        public async Task Browse()
        {
            try
            {
                ...
                if (OpenFileDialogService.ShowDialog())
                {
                    ...
                    //Block the view
                    IsBusy = true;
                    IProjectItem openedProject = await ProjectManagerService.OpenProjectAsync(filePath);
                    if (openedProject != null)
                    {
