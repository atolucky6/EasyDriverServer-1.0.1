...
{
    [Serializable]
    public class BCD : DataTypeBase
    {
        #region Constructors

        public BCD() : base("BCD", 16) { RequireByteLength = 2; }
        ...
        {
            return (ByteHelper.GetBCDAt(buffer, pos, byteOrder) * gain + offset).ToString();
        }
        ...
        public override bool TryParseToByteArray(object value, double gain, double offset, out byte[] buffer, ByteOrder byteOrder = ByteOrder.ABCD)
        {
            ...
            if (double.TryParse(value.ToString(), out double dResult))
            {
                ...
                {
                    ByteHelper.SetBCDAt(buffer, 0, result, byteOrder);
                    return true;
                }
            }
            return false;
