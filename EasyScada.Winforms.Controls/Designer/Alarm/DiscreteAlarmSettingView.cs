...
{
    public partial class DiscreteAlarmSettingView : UserControl
    {
        #region Constructors
        public DiscreteAlarmSettingView()
        {
            InitializeComponent();
        ...
        public void LoadAlarmSetting(AlarmSetting setting)
        {
            ...
            colTriggerValue.Items.Clear();
            colTriggerValue.Items.AddRange(tagPaths);
            colTriggerValue.AutoCompleteCustomSource.Clear();
            colTriggerValue.AutoCompleteCustomSource.AddRange(tagPaths);
            colTriggerValue.ButtonSpecs.Add(btnBrowserTag2);
        ...
        private void BtnBrowserTag2_Click(object sender, EventArgs e)
        {
            if (gridView.SelectedRows != null && gridView.SelectedRows.Count > 0)
            {
                ...
                {
                    gridView.EndEdit();
                    row.Cells["colTriggerValue"].Value = form.SelectedTagPath;
                    gridView.NotifyCurrentCellDirty(true);
                }
            }
        }
                ...
                if (saveFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in gridView.Rows)
                    {
                        builder.AddRow(
                            ...
                            row.Cells["colTriggerValue"].Value?.ToString(),
                    ...
                    if (lines.Length > 1)
                    {
                        ...
                        if (columns.Length == 7)
                        {
                            ...
                            for (int i = 0; i < columns.Length; i++)
                            {
                                ...
                                else if (columns[i] == "Trigger Value")
                                    colTriggerValueIndex = i;
                                ...
                                for (int i = 1; i < lines.Length; i++)
                                {
                                    ...
                                    if (rowValues.Length == 7)
                                    {
                                        ...
                                        gridView.Rows.Add(
                                            ...
                                            rowValues[colTriggerValueIndex],
