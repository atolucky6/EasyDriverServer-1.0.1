...
namespace EasyScada.Winforms.Controls.Navigator
{
                                                     ...
                                                     ISeparatorSource
	{
        ...
        protected class PageToButtonEdge : Dictionary<EasyPage, ViewDrawBorderEdge> { };
        ...
        protected class OutlookButtonSpecCollection : ButtonSpecCollection<ButtonSpec>
        {
            ...
            public OutlookButtonSpecCollection(EasyNavigator owner)
                : base(owner)
            {
            }
            #endregion
        ...
        static ViewBuilderOutlookBase()
        {
            ...
            Stream streamBlueUp = myAssembly.GetManifestResourceStream("EasyScada.Winforms.Controls.Resources.BlueUp.bmp");
            Stream streamBlueDown= myAssembly.GetManifestResourceStream("EasyScada.Winforms.Controls.Resources.BlueDown.bmp");
        ...
        public bool SeparatorMoving(Point mouse, Point splitter)
        {
            ...
            if (mouseDelta > 0)
            {
                ...
                for(int i=Navigator.Pages.Count-1; i>=0; i--)
                {
                    ...
                    EasyPage page = Navigator.Pages[i];
                    ...
                    if (_pageStackLookup.ContainsKey(page))
                    {
                        ...
                        if (checkButton.Visible)
                        {
                            ...
                            if (mouseDelta >= checkLength)
                            {
                                ...
                                checkButton.Page.SetFlags(EasyPageFlags.PageInOverflowBarForOutlookMode);
                ...
                foreach (ViewDrawNavCheckButtonBase checkButton in _pageOverflowLookup.Values)
                    if (checkButton.Visible)
                    {
                        ...
                        if (mousePos < (separatorPos - checkButtonPos))
                        {
                            ...
                            checkButton.Page.ClearFlags(EasyPageFlags.PageInOverflowBarForOutlookMode);
		...
		public override void Construct(EasyNavigator navigator, 
									   ViewManager manager,
        ...
        /// <summary>
        /// Gets the EasyPage associated with the provided view element.
        /// </summary>
        /// <param name="element">Element to search against.</param>
        /// <returns>Reference to EasyPage; otherwise null.</returns>
        public override EasyPage PageFromView(ViewBase element)
        {
            if (_pageOverflowLookup != null)
            {
                foreach (KeyValuePair<EasyPage, ViewDrawNavCheckButtonBase> pair in _pageOverflowLookup)
            ...
            {
                foreach (KeyValuePair<EasyPage, ViewDrawNavCheckButtonBase> pair in _pageStackLookup)
                    if (pair.Value == element)
                        return pair.Key;
            }
        ...
        public override void PageVisibleStateChanged(EasyPage page)
        {
            ...
            {
                bool showPageStack = page.LastVisibleSet && !page.AreFlagsSet(EasyPageFlags.PageInOverflowBarForOutlookMode);
        ...
        public override void PageEnabledStateChanged(EasyPage page)
        {
            // Reflect new state in the check button
            UpdateStatePalettes();
        ...
        /// <summary>
        /// Notification that a easy page appearance property has changed.
        /// </summary>
        /// <param name="page">Page that has changed.</param>
        /// <param name="property">Name of property that has changed.</param>
        public override void PageAppearanceChanged(EasyPage page, string property)
        {
        ...
        /// <summary>
        /// Notification that easy page flags have changed.
        /// </summary>
        /// <param name="page">Page that has changed.</param>
        /// <param name="changed">Set of flags that have changed value.</param>
        public override void PageFlagsChanged(EasyPage page, EasyPageFlags changed)
        {
            // Any change to the overflow bar setting requires a layout to effect
            if ((changed & EasyPageFlags.PageInOverflowBarForOutlookMode) == EasyPageFlags.PageInOverflowBarForOutlookMode)
                PerformNeedPaint(true);
		...
		public override void Destruct()
		{
            ...
            _events = false;
            Navigator.Pages.Inserted -= new TypedHandler<EasyPage>(OnPageInserted);
            Navigator.Pages.Removed -= new TypedHandler<EasyPage>(OnPageRemoved);
        ...
        public override bool SelectNextPage(EasyPage page, 
                                            bool wrap,
                                            bool ctrlTab)
        {
        ...
        /// <param name="ctrlTab">Associated with a Ctrl+Tab action.</param>
        /// <returns>True if new page selected; otherwise false.</returns>
        public override bool SelectPreviousPage(EasyPage page, 
                                                bool wrap,
                                                bool ctrlTab)
        {
            // There must be at least one page and allowed to select a page
        ...
        public override void PerformNextAction(DirectionButtonAction action, EasyPage page)
        {
            // Ask the header group to update the action
            action = _headerGroup.NextActionEnabled(action);
        ...
        /// <param name="action">Requested action.</param>
        /// <param name="page">Selected page at time of action request.</param>
        public override void PerformPreviousAction(DirectionButtonAction action, EasyPage page)
        {
            // Ask the header group to update the action
            action = _headerGroup.PreviousActionEnabled(action);
        ...
        public string GetOverflowButtonStates()
        {
            ...
            if (_pageOverflowLookup != null)
            {
                // There swill be an overflow button per easy page
                foreach (EasyPage page in Navigator.Pages)
                {
        ...
        public void UnshrinkAppropriatePages()
        {
            ...
            {
                // Make each visible easy page have its check button and border edge visible unless
        ...
        /// <returns></returns>
        protected virtual ViewDrawNavOutlookOverflow CreateOverflowItem(EasyPage page,
        ...
        protected virtual void ReorderCheckButtonsOverflow(EasyPage page,
        ...
        protected virtual void BringPageIntoView(EasyPage page)
        {
        ...
        protected EasyPage FirstOutlookActionPage()
        {
        ...
        /// </summary>
        /// <returns>Page for selection; otherwise null.</returns>
        protected EasyPage LastOutlookActionPage()
        {
        ...
        protected EasyPage NextOutlookActionPage(EasyPage page)
        {
            bool found = false;
        ...
        /// <param name="page">Current page to work from.</param>
        /// <returns>New page that should be selected.</returns>
        protected EasyPage PreviousOutlookActionPage(EasyPage page)
        {
            bool found = false;
        ...
        private void CreateStackItems()
        {
            ...
            // Create a check button to represent each easy page
            foreach (EasyPage page in Navigator.Pages)
            {
                // Create the draw view element for the check button and provide page it represents
                ...
                bool showPage = page.LastVisibleSet && !page.AreFlagsSet(EasyPageFlags.PageInOverflowBarForOutlookMode);
        ...
        private void OnPageInserted(object sender, TypedCollectionEventArgs<EasyPage> e)
        {
            if (!Navigator.IsDisposed && _events)
            {
                ...
                // Can we show the page as a stacking item?
                bool showPageStack = e.Item.LastVisibleSet && !e.Item.AreFlagsSet(EasyPageFlags.PageInOverflowBarForOutlookMode);
        ...
        private bool PageInTheStack(EasyPage page)
        {
        ...
        private void OnDropDownClick(object sender, EventArgs e)
        {
            if (!Navigator.InternalDesignMode)
            {
                ...
                ResetCachedEasyContextMenu();

                // Add the three standard entries
                EasyContextMenuItem moreButtons = new EasyContextMenuItem(Navigator.Outlook.TextMoreButtons, _moreButtons, new EventHandler(OnShowMoreClick));
                EasyContextMenuItem fewerButtons = new EasyContextMenuItem(Navigator.Outlook.TextFewerButtons, _fewerButtons, new EventHandler(OnShowFewerClick));
                EasyContextMenuItem addRemoveButtons = new EasyContextMenuItem(Navigator.Outlook.TextAddRemoveButtons);
                EasyContextMenuItems addRemoveButtonItems = new EasyContextMenuItems();
                ...
                _kcm.Items.Add(new EasyContextMenuItems(new EasyContextMenuItemBase[] { moreButtons, fewerButtons, addRemoveButtons }));
                ...
                foreach (EasyPage page in Navigator.Pages)
                {
                    ...
                    EasyContextMenuItem pageMenuItem = new EasyContextMenuItem(page.GetTextMapping(Navigator.Button.ContextMenuMapText),
        ...
        private void OnShowMoreClick(object sender, EventArgs e)
        {
            ...
            if (checkButton != null)
            {
                ...
                foreach (EasyPage page in _pageOverflowLookup.Keys)
                    if (_pageOverflowLookup[page] == checkButton)
                    {
                        // Remove the flag that places this page on the overflow bar
                        page.ClearFlags(EasyPageFlags.PageInOverflowBarForOutlookMode);
        ...
        private void OnShowFewerClick(object sender, EventArgs e)
        {
            ...
            foreach (ViewBase child in _viewLayout.Reverse())
                if (child.Visible && (child is ViewDrawNavOutlookStack))
                {
                    ...
                    // Search for the page that is represented by this check button
                    foreach (EasyPage page in _pageStackLookup.Keys)
                        if (_pageStackLookup[page] == checkButton)
                        {
                            // Set the flag that places this page on the overflow bar
                            page.SetFlags(EasyPageFlags.PageInOverflowBarForOutlookMode);
                            return;
                        }
                }
        ...
        private void OnCheckButtonDragRect(object sender, ButtonDragRectangleEventArgs e)
        {
            ...
            e.PreDragOffset = (Navigator.AllowPageReorder && reorderItem.Page.AreFlagsSet(EasyPageFlags.AllowPageReorder));
                ...
                if (childView.Visible && childView.ClientRectangle.Contains(e.PointOffset))
                {
                    ...
                    if (childView != reorderView)
                    {
                        ...
                        if (foundReorderView)
                        {
                            ...
                            {
                                EasyPage movePage = PageFromView(reorderView);
                                EasyPage targetPage = PageFromView(childView);
