...
{
    /// <summary>
    /// Implementation for the fixed minimize button for easy form.
    /// </summary>
    public class ButtonSpecFormWindowMin : ButtonSpecFormFixed
    {
        ...
        /// </summary>
        /// <param name="form">Reference to owning easy form instance.</param>
        public ButtonSpecFormWindowMin(EasyForm form)
        ...
        public override bool GetVisible(IPalette palette)
        {
            ...
            if (EasyForm.ApplyComposition && EasyForm.ApplyCustomChrome)
                return false;

            // The minimize button is never present on tool windows
            switch (EasyForm.FormBorderStyle)
            {
            ...
            if (!EasyForm.ControlBox)
                return false;

            // Has the minimize/maximize buttons been turned off?
            if (!EasyForm.MinimizeBox && !EasyForm.MaximizeBox)
                return false;
            ...
            if (GetViewEnabled())
            {
                ...
                if (!EasyForm.InertForm)
                {
                    ...
                    if (GetView().ClientRectangle.Contains(mea.Location))
                    {
                        ...
                        if (EasyForm.WindowState == FormWindowState.Minimized)
                            EasyForm.SendSysCommand(PI.SC_RESTORE);
                        else
                            EasyForm.SendSysCommand(PI.SC_MINIMIZE);
