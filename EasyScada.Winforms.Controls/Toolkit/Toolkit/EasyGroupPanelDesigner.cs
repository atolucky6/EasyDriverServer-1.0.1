...
{
    internal class EasyGroupPanelDesigner : EasyPanelDesigner,
                                               IEasyDesignerSelect
	{
		...
		public override void Initialize(IComponent component)
		{
            ...
            _panel = component as EasyGroupPanel;

            // Acquire service interfaces
            _selectionService = (ISelectionService)GetService(typeof(ISelectionService));

			// If inside a Easy group container then always lock the component from user size/location change
            if (_panel != null)
            {
                PropertyDescriptor descriptor = TypeDescriptor.GetProperties(component)["Locked"];
                if ((descriptor != null) && ((_panel.Parent is EasyGroup) || (_panel.Parent is EasyHeaderGroup)))
        ...
        {
            // We should only ever exist inside a Easy group container
            return ((parentDesigner is EasyGroup) || (parentDesigner is EasyHeaderGroup));
        }
		...
		public override SelectionRules SelectionRules
		{
			get
			{
				// If the panel is inside our Easy group container then prevent 
				// user changing the size or location of the group panel instance
                if ((Control.Parent is EasyGroup) || 
                    (Control.Parent is EasyHeaderGroup))
