...
namespace EasyScada.Winforms.Controls
{
	...
	public class RenderStandard : RenderBase
    {
        ...
        public override ToolStripRenderer RenderToolStrip(IPalette colorPalette)
        {
            ...
            EasyStandardRenderer renderer = new EasyStandardRenderer(colorPalette.ColorTable);
                                                         ...
                                                         int variant)
        {
            ...
            if ((rect.Width > 0) && (rect.Height > 0))
            {
                ...
                else
                {
                    ...
                    if (forBorder)
                    {
                        // Are we calculating for the outside of the border edge? This is used for a EasyForm
        ...
        private static Padding ContentPaddingForButtonForm(Padding original,
                                                           ...
                                                           int allocatedHeight)
        {
            // Get the easy form that contains this control
            EasyForm easyForm = OwningEasyForm(context.TopControl);

            // Not interested if not inside a easy form
            if (easyForm != null)
            {
                // Get the padding just for the chrome borders
                Padding chromeBorders = easyForm.RealWindowBorders;
        ...
        private static EasyForm OwningEasyForm(Control c)
        {
            // Climb chain looking for the Easy Form instance
            while ((c != null) && !(c is EasyForm))
                c = c.Parent;

            return c as EasyForm;
        }
