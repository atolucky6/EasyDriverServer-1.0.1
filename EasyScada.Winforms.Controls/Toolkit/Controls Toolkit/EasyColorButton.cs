...
{
	/// <summary>
    /// Combines color button functionality with the styling features of the Easy Toolkit.
	/// </summary>
	[ToolboxItem(true)]
    [DefaultEvent("SelectedColorChanged")]
    [DefaultProperty("SelectedColor")]
    [Designer(typeof(EasyColorButtonDesigner))]
    [ToolboxBitmap(typeof(EasyColorButton), "ToolkitBitmaps.EasyColorButton.bmp")]
    [ClassInterface(ClassInterfaceType.AutoDispatch)]
    [ComVisible(true)]
    public class EasyColorButton : VisualSimpleBase, IButtonControl, IContentValues
	{
        ...
        // Context menu items
        private EasyContextMenu _easyContextMenu;
        private EasyContextMenuSeparator _separatorTheme, _separatorStandard, _separatorRecent;
        private EasyContextMenuHeading _headingTheme, _headingStandard, _headingRecent;
        private EasyContextMenuColorColumns _colorsTheme, _colorsStandard, _colorsRecent;
        private EasyContextMenuSeparator _separatorNoColor;
        private EasyContextMenuItems _itemsNoColor;
        private EasyContextMenuItem _itemNoColor;
        private EasyContextMenuSeparator _separatorMoreColors;
        private EasyContextMenuItems _itemsMoreColors;
        private EasyContextMenuItem _itemMoreColors;
        #endregion
        ...
        /// <summary>
        /// Occurs when the value of the EasyCommand property changes.
        /// </summary>
        [Category("Property Changed")]
        [Description("Occurs when the value of the EasyCommand property changes.")]
        public event EventHandler EasyCommandChanged;
        ...
        #region Identity
        /// <summary>
        /// Initialize a new instance of the EasyColorButton class.
		/// </summary>
        public EasyColorButton()
        {
            ...
            _easyContextMenu = new EasyContextMenu();
            _separatorTheme = new EasyContextMenuSeparator();
            _headingTheme = new EasyContextMenuHeading("Theme Colors");
            _colorsTheme = new EasyContextMenuColorColumns(ColorScheme.OfficeThemes);
            _separatorStandard = new EasyContextMenuSeparator();
            _headingStandard = new EasyContextMenuHeading("Standard Colors");
            _colorsStandard = new EasyContextMenuColorColumns(ColorScheme.OfficeStandard);
            _separatorRecent = new EasyContextMenuSeparator();
            _headingRecent = new EasyContextMenuHeading("Recent Colors");
            _colorsRecent = new EasyContextMenuColorColumns(ColorScheme.None);
            _separatorNoColor = new EasyContextMenuSeparator();
            _itemNoColor = new EasyContextMenuItem("&No Color", Properties.Resources.ButtonNoColor, new EventHandler(OnClickNoColor));
            _itemsNoColor = new EasyContextMenuItems();
            _itemsNoColor.Items.Add(_itemNoColor);
            _separatorMoreColors = new EasyContextMenuSeparator();
            _itemMoreColors = new EasyContextMenuItem("&More Colors...", new EventHandler(OnClickMoreColors));
            _itemsMoreColors = new EasyContextMenuItems();
            _itemsMoreColors.Items.Add(_itemMoreColors);
            _easyContextMenu.Items.AddRange(new EasyContextMenuItemBase[] { _separatorTheme, _headingTheme, _colorsTheme, 
        ...
        /// <summary>
        /// Gets and sets the EasyContextMenu to show when right clicked.
        /// </summary>
        [Browsable(false)]
        [EditorBrowsable(EditorBrowsableState.Never)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public override EasyContextMenu EasyContextMenu
        {
            get { return null; }
            set { }
        }
        ...
        /// <summary>
        /// Gets and sets the associated EasyCommand.
        /// </summary>
        [Category("Behavior")]
        [Description("Command associated with the color button.")]
        [DefaultValue(null)]
        public virtual EasyCommand EasyCommand
        {
            get { return _command; }
            ...
            set
            {
                if (_command != value)
                {
                    ...
                    _command = value;
                    OnEasyCommandChanged(EventArgs.Empty);
        ...
        {
            if (EasyCommand != null)
                return EasyCommand.ImageTransparentColor;
            else
        ...
        /// <summary>
        /// Raises the EasyCommandChanged event.
        /// </summary>
        /// <param name="e">An EventArgs containing the event data.</param>
        protected virtual void OnEasyCommandChanged(EventArgs e)
        {
            if (EasyCommandChanged != null)
                EasyCommandChanged(this, e);

            // Use the values from the new command
            if (EasyCommand != null)
            {
                Enabled = EasyCommand.Enabled;
                Values.Image = EasyCommand.ImageSmall;
            }
        ...
        private bool ShowDropDown()
        {
            ...
            // Update the easy menu with this controls palette state
            if (_easyContextMenu != null)
            {
                if (PaletteMode != PaletteMode.Custom)
                    _easyContextMenu.PaletteMode = PaletteMode;
                else
                    _easyContextMenu.Palette = Palette;
            }
            ...
            if (!cpma.Cancel)
            {
                if (cpma.EasyContextMenu != null)
                {
                    ...
                    if (CommonHelper.ValidEasyContextMenu(cpma.EasyContextMenu))
                    {
                        ...
                        {
                            case EasyContextMenuPositionH.Before:
                                screenRect.X -= 1;
                                break;
                            case EasyContextMenuPositionH.After:
                                screenRect.Width += 1;
                                break;
                        }
                        ...
                        HookContextMenuEvents(_easyContextMenu.Items, true);
                        ...
                        cpma.EasyContextMenu.Show(this, screenRect, cpma.PositionH, cpma.PositionV);
                    }
                }
            }
        ...
        private EasyContextMenuPositionH GetPositionH()
        {
            switch (DropDownOrientation)
            {
                ...
                case VisualOrientation.Bottom:
                case VisualOrientation.Top:
                    return EasyContextMenuPositionH.Left;
                case VisualOrientation.Left:
                    return EasyContextMenuPositionH.Before;
                case VisualOrientation.Right:
                    return EasyContextMenuPositionH.After;
            }
        }

        private EasyContextMenuPositionV GetPositionV()
        {
            switch (DropDownOrientation)
            {
                default:
                case VisualOrientation.Bottom:
                    return EasyContextMenuPositionV.Below;
                case VisualOrientation.Top:
                    return EasyContextMenuPositionV.Above;
                case VisualOrientation.Left:
                case VisualOrientation.Right:
                    return EasyContextMenuPositionV.Top;
            }
        }
        ...
        private void OnEasyContextMenuClosed(object sender, EventArgs e)
        {
            ...
            HookContextMenuEvents(_easyContextMenu.Items, false);
        }
        ...
        private void HookContextMenuEvents(EasyContextMenuCollection collection, bool hook)
        {
            // Search for items of interest
            foreach (EasyContextMenuItemBase item in collection)
            {
                // Hook into color events
                if (item is EasyContextMenuColorColumns)
                {
                    EasyContextMenuColorColumns columns = (EasyContextMenuColorColumns)item;
        ...
        private void UpdateRecentColors(Color color)
        {
            ...
            if (AutoRecentColors)
            {
                ...
                foreach (EasyContextMenuItemBase item in _easyContextMenu.Items)
                {
                    // Only interested in the non-recent colors color columns
                    if ((item is EasyContextMenuColorColumns) && (item != _colorsRecent))
                    {
                        // Cast to correct type
                        EasyContextMenuColorColumns colors = (EasyContextMenuColorColumns)item;
        ...
        private void DecideOnVisible(EasyContextMenuItemBase visible, EasyContextMenuItemBase target)
        {
            bool previous = false;
            ...
            if (target.Visible)
            {
                ...
                foreach (EasyContextMenuItemBase item in _easyContextMenu.Items)
                {
                    ...
                    if (!((item is EasyContextMenuSeparator) ||
                          (item is EasyContextMenuHeading)))
                    {
