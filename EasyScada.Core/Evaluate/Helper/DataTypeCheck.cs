...
namespace EasyScada.Core.Evaluate
{
    public class DataTypeCheck
    {
        ...
        public static string[] OperandFunctions = { "tag", "avg", "abs", "iif", "lcase", "left", "len", "mid", "right", "round", "sqrt", "ucase", "isnullorempty", "istrueornull", "isfalseornull", "trim", "rtrim", "ltrim", "dateadd", "concat", "date", "rpad", "lpad", "join", "searchstring", "day", "month", "year", "substring", "numericmax", "numericmin", "datemax", "datemin", "stringmax", "stringmin", "contains", "between", "indexof", "now", "replace", "eval", "remove", "quote", "pcase", "sin", "cos", "not", "isalldigits" };
        ...
        public static bool ContainsOperator(string CheckString, out string sOperand, out string sOperator)
        {
            ...
            for (int i = 0; i < DataTypeCheck.ArithOperators.Length; i++)
            {
                if (CheckString.EndsWith(DataTypeCheck.ArithOperators[i]) == true)
                {
                    containsOperator = true;
                    sOperator = DataTypeCheck.ArithOperators[i];
            ...
            for (int i = 0; i < DataTypeCheck.ComparisonOperators.Length; i++)
            {
                if (CheckString.EndsWith(DataTypeCheck.ComparisonOperators[i]) == true)
                {
                    containsOperator = true;
                    sOperator = DataTypeCheck.ComparisonOperators[i];
                    sOperand = CheckString.Substring(0, CheckString.Length - sOperator.Length);
                    break;
                }
            }
